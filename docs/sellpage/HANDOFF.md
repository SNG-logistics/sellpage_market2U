...
| A13 Mobile first | Editor opens at Mobile 390 (`ui` prop on `<Puck>`; Puck otherwise picks the size nearest the editor window — Desktop 1440). New pages default to a 480px column; the null-theme fallback for old published pages is pinned at 960 (`legacyPublishedTheme`). |
